...
func (b *buffer) readHexString() (token, error) {
	...
	Loop2:
		...
		if x < 0 {
			return nil, fmt.Errorf("malformed hex string %c %c %s", c, c2, b.buf[b.pos:])
		}
	...
	for !b.eof {
		...
		case '\\':
			...
			case '0', '1', '2', '3', '4', '5', '6', '7':
				...
				if x > 255 {
					return nil, fmt.Errorf("invalid octal escape \\%03o", x)
				}
				tmp = append(tmp, byte(x))
			}
		}
...
func (b *buffer) readKeyword() (token, error) {
	...
	case isReal(s):
		...
		if err != nil {
			return nil, fmt.Errorf("invalid real %s", s)
		}
		return x, nil
	}
